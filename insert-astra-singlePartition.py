...
# Insert Bind
ticket_insert = "INSERT INTO " + ksname + "." + tblname + " (my_id, id, ownedby, ticket, time, notes) VALUES (?, ?, ?, ?, ?, ?)"
ticketins_prep = session.prepare(ticket_insert, ConsistencyLevel.QUORUM)
# Select Bind
ticket_select = """
    SELECT my_id, id,  ownedby, ticket, time, notes
    FROM """ + ksname + "." + tblname + """
    WHERE my_id = ?
    """
...
for myrange in range (1,2):
    ...
    for i in range (startval,endval):
        ...
        ### Bound statement
        tickins_bind = ticketins_prep.bind((myrange, i, pickowner, pickticket, timenow, datastr))
        ...
        if i in power10:
            ...
            ### READ SCENARIO
            ticksel_bind = ticketsel_prep.bind([myrange])
            selrows = session.execute(ticksel_bind)
            
            for row in selrows:
                print(row.my_id, row.id, row.ownedby, row.ticket, row.time, row.notes)
            
            tickselcnt_bind = ticketselcnt_prep.bind([myrange])
            ...
            saiSimplestmt = SimpleStatement( "SELECT my_id, id,  ownedby, ticket, time, notes FROM " + ksname + "." + tblname + " WHERE my_id = %s AND time >= %s AND time <= %s;" % (myrange, min_time, timenow) ,
            ...
            for sai_row in sai_rows:
                print(sai_row.my_id, sai_row.id, sai_row.ownedby, sai_row.ticket, sai_row.time, sai_row.notes)
            print( "SAI READ " + str(i) + " FINISHED " + str( helper.dtn() - startsairead ) )

            # saicntSimplestmt = SimpleStatement( "select count(*) FROM " + ksname + "." + tblname + " WHERE my_id = %s AND time >= %s AND time <= %s;" % (myrange, min_time, timenow) ,
            #     consistency_level=ConsistencyLevel.QUORUM)
